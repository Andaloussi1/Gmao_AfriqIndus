    ...
    <AppLayout title="Articles">
        ...
        <Table :resource="articles">
            <template #cell(actions)="{ item: article }">
                <Link :href="route('articles.edit',article.id)" class="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded">Modifier
                </Link>
                <button @click="destroy(article.id)" class=" focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-3 py-2 mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Supprimer
                </button>
                <Link :href="route('articles.show',article.id)" type="button" class="text-blue-700 border border-blue-700 hover:bg-blue-700 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center mr-2 dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:focus:ring-blue-800">
